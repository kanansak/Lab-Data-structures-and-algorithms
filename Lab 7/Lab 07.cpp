...
void listEdge(){
	...
	while(data!=NULL){
		printf("Edge %s : %s",data->node1,data->node2);
		data = data->next;
	}	
	printf("\n");
}
...
void enpueuePath(char nodedata[100]){
	struct Edge *edgeData = edge;
	while(edgeData!=NULL){
		if(strcmp(edgeData->node1,nodedata)==0){
			struct Node *nodeTest = getNode(edgeData->node2);
			if(!nodeTest->mark){
				if(!isInQ(edgeData->node2)){
					enqueue(edgeData->node2);
				}
			}
		}else if (strcmp(edgeData->node2,nodedata)==0){
			struct Node *nodeTest = getNode(edgeData->node1);
